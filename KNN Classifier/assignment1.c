...
void calculateDistance(float arr[][210][210],float arr2[210][8], int size){
...
int main(int argc, char const *argv[])
{
	...
	int shufflearr[210];
	populate(shufflearr, 210);
	randomize(shufflearr, 210);
	...
	float data[210][8]; //initialize the data array .. complete data will be strored here
	int count = 0;
	int row = -1;
	...
	float distances[5][210][210];
	calculateDistance(distances,data,210);
		...
		for(int p = 0; p < 5; p++){
			for(int temp1 = 30*r; temp1 < 30*(r+1); temp1++){
				int count = 0;
				{
					...
					for(int dtemp = 30*(r+1); dtemp < 210; dtemp++){
